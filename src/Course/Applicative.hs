...
lift4 g xs ys zs ws = g <$> xs <*> ys <*> zs <*> ws
...
sequence = foldRight (\xs acc -> (:.) <$> xs <*> acc) (pure Nil)
...
-- (I had to look this one up. ~daniel)
filtering :: Applicative f => (a -> f Bool) -> List a -> f (List a)
filtering p = foldRight step (pure Nil)
  where
    step x acc = (\p' -> if p' then (x :.) else id) <$> (p x) <*> acc
